...
(function () {
  ...
  const TITLE_TO_ID = PROMOS.reduce((map, promo) => {
    map[promo.title] = promo.id;
    return map;
  }, {});
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      #container section.promotion-detail-section{background:#0f1419!important;min-height:calc(100dvh - 64px)!important;padding-top:40px!important;padding-bottom:72px!important}
      ...
      .promotion-detail-logo{height:42px;object-fit:contain;mix-blend-mode:lighten}.promotion-detail-kicker{margin:0;color:#98E7D2;font-size:14px;font-weight:800;letter-spacing:.22em;text-transform:uppercase}.promotion-detail-title{margin:8px 0 0;font-size:50px;font-weight:800;line-height:1.08;background:linear-gradient(90deg,#CBE8E4,#98E7D2);-webkit-background-clip:text;background-clip:text;color:transparent}
      .promotion-detail-subtitle{margin:12px 0 0;color:#fff;font-size:22px;font-weight:800}.promotion-detail-offers{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px;width:100%;margin-top:2px}.promotion-detail-offer{position:relative;border:1px solid rgba(152,231,210,.24);border-radius:8px;background:#0f1419;padding:28px 18px 22px;box-shadow:inset 0 1px 0 rgba(255,255,255,.03)}
      ...
      .promotion-detail-offer h4{margin:0 0 10px;color:#CBE8E4;font-size:19px;font-weight:800}.promotion-detail-offer strong{display:block;color:#fff;font-size:30px;line-height:1}.promotion-detail-offer span{display:block;margin-top:10px;color:#9ca3af;font-size:14px;font-weight:700}
      .promotion-detail-rules{width:100%;margin:0;padding:22px 26px;border:1px solid rgba(152,231,210,.18);border-radius:8px;background:rgba(15,20,25,.78);text-align:left}.promotion-detail-rules h4{margin:0 0 12px;text-align:center;color:#CBE8E4;font-size:18px}.promotion-detail-rules ol{margin:0;padding-left:18px;color:#d1d5db;font-size:15px;line-height:1.7}.promotion-detail-note{margin:0;color:#9ca3af;font-size:13px;line-height:1.6}.promotion-detail-cta{width:min(100%,420px);border:0;border-radius:8px;background:linear-gradient(90deg,#CBE8E4,#98E7D2);color:#07131c;font-size:17px;font-weight:800;padding:14px 18px;cursor:pointer;transition:opacity .16s ease}.promotion-detail-cta:hover,.promotion-detail-cta:focus-visible{opacity:.88;outline:none}
      @media(max-width:768px){#container section.promotion-detail-section{padding-top:28px!important}.promotion-detail-back{font-size:16px;margin-bottom:22px}.promotion-detail-heading{font-size:28px}.promotion-detail-poster{padding:36px 22px 32px}.promotion-detail-poster:before{inset:12px}.promotion-detail-content{min-height:640px}.promotion-detail-title{font-size:36px}.promotion-detail-subtitle{font-size:19px}.promotion-detail-offers{grid-template-columns:1fr;gap:22px}.promotion-detail-rules{padding:18px 20px}.promotion-detail-rules ol{font-size:14px}}
    `;
  ...
  function renderDetail(id) {
    ...
    target.innerHTML = `
      <div class="promotion-detail-page">
        <button class="promotion-detail-back cms-back-button" type="button" aria-label="Back">
  ...
  // 首頁 Promotion 卡片:點 Detail 進對應詳情頁
  function bindHomeCards() {
    [...document.querySelectorAll('#container button')]
      .filter((b) => (b.textContent || '').trim() === 'Detail' && !b.dataset.promotionBound)
      .forEach((btn) => {
        let box = btn.parentElement;
        let title = null;
        while (box && !title) {
          title = [...box.querySelectorAll('h3')].map((h) => h.textContent.trim()).find((t) => TITLE_TO_ID[t]);
          if (!title) box = box.parentElement;
        }
        if (!title) return;
        btn.dataset.promotionBound = 'true';
        btn.type = 'button';
        btn.addEventListener('click', (event) => {
          event.preventDefault();
          goToDetail(TITLE_TO_ID[title]);
        });
      });
  }
  ...
  document.addEventListener('page:rendered', (event) => {
    ...
    if (detail.slug === 'home') { bindHomeCards(); return; }
