      ...
      <div class="flex flex-col gap-2">
        ...
        >
          <div class="mb-2 flex items-center justify-between">
            <span class="truncate text-[10px] text-ink-4">{{ m.league }}</span>
            <span class="flex items-center gap-1 rounded border border-danger/30 bg-danger/20 px-1.5 py-0.5 text-[10px] text-danger">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-2.5 w-2.5 animate-pulse">
                <path d="M4.9 19.1C1 15.2 1 8.8 4.9 4.9" />
                <path d="M7.8 16.2c-2.3-2.3-2.3-6.1 0-8.5" />
                <circle cx="12" cy="12" r="2" />
                <path d="M16.2 7.8c2.3 2.3 2.3 6.1 0 8.5" />
          ...
          <div class="flex items-center justify-between gap-3">
            <div class="flex min-w-0 flex-1 items-center gap-2">
              <div class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full border border-line bg-surface-deep">
                <span class="text-[10px] font-semibold text-ink-2">{{ m.teamA.code }}</span>
              </div>
              <span class="truncate text-xs text-ink">{{ m.teamA.name }}</span>
            </div>
            <div class="flex-shrink-0 px-2 text-center">
              <div class="text-base font-bold leading-none text-primary">{{ m.score }}</div>
              <div class="mt-0.5 whitespace-nowrap text-[10px] text-ink-4">{{ m.minute }}</div>
            </div>
            <div class="flex min-w-0 flex-1 items-center justify-end gap-2">
              <span class="truncate text-right text-xs text-ink">{{ m.teamB.name }}</span>
              <div class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full border border-line bg-surface-deep">
                <span class="text-[10px] font-semibold text-ink-2">{{ m.teamB.code }}</span>
              </div>
            </div>
          </div>
        </div>
