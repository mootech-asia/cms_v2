...
- [~] 2 共用元件(部分完成)
  ...
  - [x] **語言切換觸發鈕 `.lang-switch`**(全 21 頁 header + site.js `localeTriggerHtml`,32×;`.relative`/`.dd-panel`/
    `[data-locale-option]` hook 全保留)parity real-diff 0(index 9 筆為 marquee transform/pulse opacity 動畫非決定性,
    clean-vs-clean 對照重現)、9/9、28/28。
  - [ ] 大廳 app-header 內層(皮膚切換觸發/帳號列/分類 nav 其餘 — 較複雜)。
